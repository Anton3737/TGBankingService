...
public class PutMarks<T> {
  ...
  public InlineKeyboardMarkup addButtons(List<T> buttonTexts, List<String> userParams, int rows) {
    InlineKeyboardMarkup.InlineKeyboardMarkupBuilder inlineKeyboardMarkup =
    ...
    for (T button : buttonTexts) {
      ...
      InlineKeyboardButton buttonObject =
          InlineKeyboardButton.builder().text(finalText).callbackData(text).build();
